...
一个Skill就是一个文件夹，里面必须有一个 `SKILL.md` 文件，这是核心。除此之外，你还可以放一些配套文件，比如参考文档、示例模板、甚至Python或Bash脚本
...
- `disable-model-invocation: true`，加了之后，Claude就不会自动触发这个Skill了，只有你手动输入斜杠命令才会执行

#### 测试 skill

打开一个 git 项目，对任何文件进行小的编辑，并通过运行 `claude` 启动 Claude Code。你可以通过两种方式测试该 skill。让 Claude 自动调用它，通过询问与描述匹配的内容：
...
可以使用这个skill帮助创建skill

### 去哪找更多Skill

#### 官方插件市场

在Claude Code对话框里输入 `/plugin`，就能打开官方的插件市场，里面有Anthropic官方和社区认证的插件，质量有保障，这是最推荐的渠道。

#### 第三方Skill分享网站

比如skillsmp、skills.sh、agentskill.sh、Lobehub Skills

#### github

直接在GitHub上搜”claude code skill”或者”claude code plugin”
